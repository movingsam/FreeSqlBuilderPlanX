 ...
 /// </summary>
    public static class Web
    {
        ...
        public static void Init(IServiceProvider service)
        {
            ...
            Environment = service.GetService<IWebHostEnvironment>();
        }
        #endregion
        ...
        /// 检测上传文件是否包含非图片文件
        /// </summary>
        public static void CheckFileIsImage()
        {
            var files = Request.Form.Files;
            if (files?.Where(file => file?.Length > 0).Any(file => File.GetFileType(file) != "Image") ?? false) throw new Warning("不允许上传非图片文件");
        }
